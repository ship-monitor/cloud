...
func HandleCreateInvitation(c *gin.Context) {
	...
	if err != nil {
		log.Warn("Access denied for create invitation — not a member", "organization", orgID, "user", session.UserID)
	...
	if member.Role != data.RoleOwner && member.Role != data.RoleAdministrator {
		log.Warn("Insufficient role for create invitation", "organization", orgID, "user", session.UserID, "role", member.Role)
	...
	if err != nil {
		log.Warn("Failed to read invitation request body", "error", err, "organization", orgID, "user", session.UserID)
	...
	if err := json.Unmarshal(body, &singleReq); err == nil && singleReq.InviteeEmail != "" {
		...
		if err != nil {
			log.Warn("Failed to create invitation", "error", err, "organization", orgID, "email", singleReq.InviteeEmail)
	...
	if err := json.Unmarshal(body, &bulkReq); err == nil && len(bulkReq.InviteeEmails) > 0 {
		...
		for _, email := range bulkReq.InviteeEmails {
			...
			if err != nil {
				log.Warn("Failed to create invitation in bulk", "error", err, "organization", orgID, "email", email)
...
func HandleListOrgInvitations(c *gin.Context) {
	...
	member, err := data.GetMember(orgID, session.UserID)
	if err != nil {
		log.Warn("Access denied for list org invitations — not a member", "organization", orgID, "user", session.UserID)
		c.JSON(http.StatusForbidden, dto.Error(errors.New("access denied")))
		return
	}
	if member.Role != data.RoleOwner && member.Role != data.RoleAdministrator {
		log.Warn("Insufficient role for list org invitations", "organization", orgID, "user", session.UserID, "role", member.Role)
	...
	if err != nil {
		log.Error("Failed to list org invitations", "error", err, "organization", orgID, "user", session.UserID)
		c.JSON(http.StatusInternalServerError, dto.Error(err))
		return
	}
...
func HandleAcceptInvitation(c *gin.Context) {
	...
	if inv.Status != data.StatusPending {
		log.Warn("Attempt to accept already processed invitation", "invitation", invID, "status", inv.Status)
		c.JSON(http.StatusBadRequest, dto.Error(errors.New("invitation already processed")))
		return
	}

	if time.Now().After(inv.ExpiresAt) {
		log.Warn("Attempt to accept expired invitation", "invitation", invID, "expiresAt", inv.ExpiresAt)
	...
	if session.Email != inv.InviteeEmail {
		log.Warn("User is not the invitee", "invitation", invID, "user", session.UserID, "invitee", inv.InviteeEmail)
	...
	if err != nil {
		log.Error("Failed to check membership on accept", "error", err, "invitation", invID, "user", session.UserID)
		c.JSON(http.StatusInternalServerError, dto.Error(err))
		return
	}
	if alreadyMember {
		log.Warn("User is already a member on accept", "invitation", invID, "user", session.UserID, "organization", inv.OrganizationID)
	...
	if err := data.UpdateInvitationStatus(invID, data.StatusAccepted); err != nil {
		log.Error("Failed to update invitation status to accepted", "error", err, "invitation", invID)
		c.JSON(http.StatusInternalServerError, dto.Error(err))
		return
	}
	...
	if err := data.AddMember(data.OrganizationMember{
		...
		log.Error("Failed to add member after invitation accept", "error", err, "invitation", invID, "user", session.UserID, "organization", inv.OrganizationID)
		c.JSON(http.StatusInternalServerError, dto.Error(err))
		return
	}
...
func HandleDeclineInvitation(c *gin.Context) {
	...
	if inv.Status != data.StatusPending {
		log.Warn("Attempt to decline already processed invitation", "invitation", invID, "status", inv.Status)
		c.JSON(http.StatusBadRequest, dto.Error(errors.New("invitation already processed")))
		return
	}
	...
	session := auth.GetSession(c)
	if session.Email != inv.InviteeEmail {
		log.Warn("User is not the invitee on decline", "invitation", invID, "user", session.UserID, "invitee", inv.InviteeEmail)
		c.JSON(http.StatusForbidden, dto.Error(errors.New("you are not the invitee")))
		return
	}

	if err := data.UpdateInvitationStatus(invID, data.StatusDeclined); err != nil {
		log.Error("Failed to update invitation status to declined", "error", err, "invitation", invID)
		c.JSON(http.StatusInternalServerError, dto.Error(err))
		return
	}
